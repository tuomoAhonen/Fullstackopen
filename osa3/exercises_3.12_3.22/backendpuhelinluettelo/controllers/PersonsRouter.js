...
const nameLengthError = `Error: Name must be atleast 3 characters!`;
const nameTypeError = `Error: Name must be a string!`;

const PersonsRouter = (app) => {
  ...
  app.post('/api/persons', async (request, response, next) => {
    if (nameValidation(request.body.name)) {
      if (phoneValidation(request.body.phone)) {
        ...
        console.log(foundPerson, foundPerson2)
        if (foundPerson === null && foundPerson2 === null) {
          ...
          switch (true) {
            case foundPerson === null:
              if (typeof foundPerson2 === 'string') {
                return next(foundPerson2);
              } else {
                return next(`Error: This number is already in your phonebook on person: ${foundPerson2.name}, phone: ${foundPerson2.phone}`);
              }
            case foundPerson2 === null:
              if (typeof foundPerson === 'string') {
                return next(foundPerson);
              } else {
                return next(`Error: This name is already in your phonebook: ${foundPerson.name} with phone: ${foundPerson.phone}`);
              }
            case typeof foundPerson === 'string' && typeof foundPerson2 === 'string':
              return next(`Error: Name & phone inputs must be strings!`);
            case 'id' in foundPerson && 'id' in foundPerson2:
              if (foundPerson.id === foundPerson2.id) {
                return next(`Error: This person is already in your phonebook: ${foundPerson.name}, phone: ${foundPerson.phone}`);
              } else {
                return next(`Error: Name & phone were already in your phonebook on different persons`);
              }
            default:
              return next(`Error: Something went horrible wrong, SOS!`);
          }
        }
      } else {
      ...
      if (typeof request.body.name !== 'string') {
        return next(`${nameTypeError}`);
      } else {
        return next(`${nameLengthError}`);
      }
    }
  });
