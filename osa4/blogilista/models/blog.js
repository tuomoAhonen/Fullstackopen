...
const Blog = sequelize.define('blog', {
  ...
  createdbyuser: {
    ...
    get() {
      if (this.createdbyuserid !== null && typeof this.createdbyuserid === 'number'/* && this.createdbyuserid.toString().match(/^(\d{1,255})/gi)*/) {
        return (async () => {
          //infolog(this.createdbyuserid);
          const result = User.findOne({attributes: ['userid', 'username', 'name'], where: {userid: this.createdbyuserid}});
          return result;
        })();
      } else {
        return null;
